...
export function buildDoctorReport(args: {
  ...
  const checks: DoctorCheck[] = [
    ...
    check(
      ...
      "Run `agent-lcm health --json`; if the index is corrupt, move it aside and re-import or let hooks rebuild it.",
...
function summaryIndexCheck(health: Health): DoctorCheck {
  ...
  return check(
    ...
    "Run `agent-lcm stats --json`; new high-signal events should rebuild summaries automatically.",
  );
}
