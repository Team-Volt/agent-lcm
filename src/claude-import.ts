...
export type ClaudeSessionReadResult = {
  ...
  readonly errors: readonly ClaudeImportError[];
};
...
export async function readClaudeSession(file: string): Promise<ClaudeSessionReadResult> {
  const events: NormalizedEvent[] = [];
  const errors: ClaudeImportError[] = [];
...
function recordEvents(
  ...
  if (!message) return [];
  if (type === "user") return userEvents(record, message, context);
  if (type === "assistant") return assistantEvents(record, message, context);
  return [];
}
...
function userEvents(
  ...
  if (record.isMeta === true || stringValue(message.role) !== "user") return [];
  ...
  if (blocks.some((block) => block.type === "tool_result")) return results;
...
function assistantEvents(
  ...
  if (stringValue(message.role) !== "assistant") return [];
  const content = message.content;
  const blocks = Array.isArray(content) ? content.filter(isRecord) : [];
  for (const block of blocks) {
    ...
    if (id && name) context.pendingTools.set(id, { name, input: block.input });
  }
...
function metadata(record: Record<string, unknown>, file: string): Record<string, unknown> {
  return {
...
function textBlocks(blocks: readonly Record<string, unknown>[]): string | undefined {
...
function recordTimestamp(record: Record<string, unknown>): string {
  ...
  if (Number.isNaN(new Date(value).getTime())) throw new Error(`invalid timestamp: ${value}`);
  return value;
}
