...
import { generateOpenCodePluginSource } from "./opencode-plugin.ts";
import { isOwnedOpenCodeMcp, mutateOpenCodeMcp, openCodeJsoncExists, readOpenCodeConfigurationSnapshot, validateOpenCodeConfiguration } from "./setup-opencode-config.ts";
import { ensureSetupDirectory, mutateSetupFile, readSetupConfiguration, readSetupFileBytes } from "./setup-files.ts";
import { assertSafeSetupCommand } from "./setup-hook-status.ts";
import { openCodeConfigPath, openCodeJsoncPath, openCodeStatePath, setupPath } from "./setup-targets.ts";
import type { HarnessSetupStatus, RemoveOptions, RemoveReport, SetupOptions, SetupReport, SetupStatusOptions } from "./setup.ts";
...
type OpenCodeSource = { readonly path: string; readonly bytes: Buffer; readonly command: string; readonly hash: string };
type OpenCodePluginPreflight = { readonly sources: readonly OpenCodeSource[]; readonly stateBytes: Buffer | undefined; readonly stateCommand: string | undefined };

export function setupOpenCode(options: SetupOptions): SetupReport {
  ...
  if (jsoncPath !== undefined) validateOpenCodeConfiguration(jsoncSnapshot?.configuration, jsoncPath, "setup", priorCommand ?? command);
  ...
  if (jsoncPath !== undefined) validateOpenCodeConfiguration(jsoncSnapshot?.configuration, jsoncPath, "setup", lockedPriorCommand ?? command);
...
export function removeOpenCode(options: RemoveOptions): RemoveReport {
  const target = setupPath("opencode", options.home);
  const configPath = openCodeConfigPath(options.home);
  const statePath = openCodeStatePath(options.home);
  const jsoncPath = openCodeJsoncExists(options.home) ? openCodeJsoncPath(options.home) : undefined;
  ...
  if (jsoncPath !== undefined) validateOpenCodeConfiguration(jsoncSnapshot?.configuration, jsoncPath, "remove", priorCommand);
  let captureChanged = false;
  let mcpChanged = false;
  let captureCompleted = false;
  let mcpCompleted = false;
  try {
    if (plugin.sources.length > 0) captureChanged = upgradeOpenCodeSources(plugin.sources, target);
...
export function openCodeSetupStatus(options: SetupStatusOptions): HarnessSetupStatus {
  const target = setupPath("opencode", options.home);
  ...
  let pluginCommand: string | undefined;
  try {
    if (pluginBytes !== undefined) pluginCommand = managedOpenCodeCommand(pluginBytes, target, "status");
  } catch {
...
function openCodeSetupReport(target: string, mcpPath: string, captureChanged: boolean, mcpChanged: boolean): SetupReport {
...
function openCodeRemoveReport(target: string, mcpPath: string, captureChanged: boolean, mcpChanged: boolean): RemoveReport {
...
function effectiveOpenCodeConfiguration(home?: string): Record<string, unknown> | undefined {
  const json = readConfigurationForStatus(openCodeConfigPath(home));
  if (!openCodeJsoncExists(home)) return json;
  const jsonc = readOpenCodeConfigurationForStatus(openCodeJsoncPath(home));
  if (jsonc === undefined) return undefined;
  const jsonMcp = isRecord(json?.mcp) ? json.mcp : {};
  const jsoncMcp = isRecord(jsonc.mcp) ? jsonc.mcp : {};
  return { mcp: { ...jsonMcp, ...jsoncMcp } };
}

function preflightOpenCodePlugin(target: string, statePath: string, action: "setup" | "remove"): OpenCodePluginPreflight {
  const directory = path.dirname(target);
  if (!directoryExists(directory)) return { sources: [], stateBytes: undefined, stateCommand: undefined };
  const sources: OpenCodeSource[] = [];
  const existing = readSetupFileBytes(target);
  if (existing !== undefined) sources.push({ path: target, bytes: existing, command: managedOpenCodeCommand(existing, target, action), hash: openCodeBytesHash(existing) });
  for (const name of fs.readdirSync(directory)) {
    if (!/^agent-lcm-pre-agent-lcm-[A-Za-z0-9-]+\.ts$/u.test(name)) continue;
    const candidate = path.join(directory, name);
    const status = fs.lstatSync(candidate);
    if (status.isSymbolicLink()) throw new Error(`Refusing OpenCode plugin backup symlink: ${candidate}`);
...
function upgradeOpenCodeSources(sources: readonly OpenCodeSource[], target: string, desiredTarget?: Buffer): boolean {
  let changed = false;
  ...
  if (desiredTarget !== undefined && (targetSource === undefined || !targetSource.bytes.equals(desiredTarget))) {
    changed = mutateSetupFile(target, (current) => {
      if (current !== undefined && !isKnownManagedOpenCodePlugin(current)) throw new Error(`Refusing to overwrite unmanaged OpenCode plugin: ${target}`);
  ...
  for (const source of sources) {
    ...
    if (source.bytes.equals(desired)) continue;
    changed = mutateSetupFile(source.path, (current) => {
      if (current !== undefined && !isKnownManagedOpenCodePlugin(current)) throw new Error(`Refusing to overwrite unmanaged OpenCode plugin backup: ${source.path}`);
      return desired;
...
function setOpenCodeState(target: string, state: "enabled" | "disabled", command: string | undefined, existing?: Buffer): boolean {
  if (command === undefined) return false;
  const desired = Buffer.from(`${state}\n${JSON.stringify(command)}\n`, "utf8");
  return mutateSetupFile(target, (current) => {
    if (current !== undefined) managedOpenCodeStateCommand(current, target);
    return desired;
  }, existing === undefined ? undefined : openCodeBytesHash(existing));
}

function throwOpenCodePartialFailure(action: "setup" | "remove", mcpCompleted: boolean, captureCompleted: boolean, error: unknown): never {
...
function isKnownManagedOpenCodePlugin(bytes: Buffer | undefined): boolean {
...
function isLegacyOpenCodePlugin(source: string): boolean {
  if (!source.startsWith("// Generated by Agent LCM. Do not edit.\n")) return false;
  const commandLine = /^const AGENT_LCM_COMMAND = .*;$/mu;
  if (!commandLine.test(source)) return false;
  const normalize = (value: string): string => value.replace(commandLine, `const AGENT_LCM_COMMAND = ${JSON.stringify(OPEN_CODE_COMMAND_SENTINEL)};`);
...
function managedOpenCodeCommand(bytes: Buffer, target: string, action: "setup" | "remove" | "status"): string {
  const command = openCodeCommandFromSource(bytes.toString("utf8"));
  if (command === undefined || !isKnownManagedOpenCodePlugin(bytes)) throw new Error(`Refusing to ${action} unmanaged OpenCode plugin: ${target}`);
  return command;
}

function matchesGeneratedOpenCodePlugin(source: string, command: string): boolean {
...
function previousOpenCodePluginSource(source: string): string {
...
function openCodeCommandFromSource(source: string): string | undefined {
...
function managedOpenCodeStateCommand(bytes: Buffer, target: string): string {
  try {
...
function isOpenCodeStateEnabled(bytes: Buffer, command: string | undefined): boolean {
  if (!bytes.toString("utf8").startsWith("enabled\n") || command === undefined) return false;
  try { return managedOpenCodeStateCommand(bytes, "state marker") === command; } catch { return false; }
}
function openCodeBytesHash(bytes: Buffer): string { return createHash("sha256").update(bytes).digest("hex"); }
function withOpenCodeMarker(source: string): string { return source.startsWith(`${OPEN_CODE_MARKER}\n`) ? source : `${OPEN_CODE_MARKER}\n${source}`; }
function directoryExists(directory: string): boolean {
  try { return requireDirectory(directory).isDirectory(); } catch (error) {
    if (error instanceof Error && Reflect.get(error, "code") === "ENOENT") return false;
    throw error;
  }
}
function requireDirectory(directory: string): fs.Stats {
  const status = fs.lstatSync(directory);
  if (!status.isDirectory()) throw new Error(`Cannot use setup path through a non-directory: ${directory}`);
  return status;
}
function readConfigurationForStatus(target: string): Record<string, unknown> | undefined { try { return readSetupConfiguration(target); } catch { return undefined; } }
function readOpenCodeConfigurationForStatus(target: string): Record<string, unknown> | undefined { try { return readOpenCodeConfigurationSnapshot(target, "setup").configuration; } catch { return undefined; } }
function readRawConfigurationForStatus(target: string): Buffer | undefined { try { return readSetupFileBytes(target); } catch { return undefined; } }
