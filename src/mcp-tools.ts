...
export function callTool(storage: LcmStorage, params: Record<string, unknown>): ReturnType<typeof toolResult> {
  ...
  switch (name) {
    ...
    case "lcm_grep": {
      ...
      return toolResult(
        `Found ${matches.length} LCM matches and ${overflowMatches.length} overflow matches. Search scope: ${searchScope}.`,
        { matches, overflow_matches: overflowMatches, search_scope: searchScope },
      );
    }
    case "lcm_describe": {
