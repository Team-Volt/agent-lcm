...
function processInputBuffer(
  ...
  handle: (raw: string, framing: "line" | "header") => void,
...
async function handleRawMessage(raw: string, framing: "line" | "header"): Promise<void> {
...
async function handleMessage(message: JsonRpcRequest, framing: "line" | "header"): Promise<void> {
  ...
  if (id !== undefined) sendError(id, -32601, `Method not found: ${method ?? ""}`, framing);
}


function send(message: unknown, framing: "line" | "header" = "line"): void {
...
function sendResult(id: JsonRpcRequest["id"], result: unknown, framing: "line" | "header" = "line"): void {
  if (id === undefined) return;
  send({ jsonrpc: "2.0", id, result }, framing);
}

function sendError(id: JsonRpcRequest["id"], code: number, message: string, framing: "line" | "header" = "line"): void {
  if (id === undefined) return;
