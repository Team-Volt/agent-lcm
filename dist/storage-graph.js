...
import { countMap, extractEventMetadata } from "./storage-sessions.js";
import { getSummaryNodesForGraph } from "./storage-summaries.js";
import { summaryNodeTitle } from "./summary.js";
export const CHECKPOINT_INTERVAL = 50;
function summaryNodeToGraphNode(node) {
    return {
...
function turnNodeId(sessionId, turnId) {
    return `turn:${sessionId}:${turnId}`;
}
function eventNodeId(eventId) {
    return `event:${eventId}`;
}
function checkpointNodeId(sessionId, eventCount) {
    return `checkpoint:${sessionId}:${eventCount}`;
}
function checkpointGraphNode(event, eventCount, metadata) {
    return {
...
function buildFallbackGraph(events, limit) {
    const sessionId = events[0]?.session_id ?? "";
    const nodes = [];
    const edges = [];
    const seen = new Set();
    const edgeKeys = new Set();
    const preToolEvents = new Map();
    const addNode = (node) => {
        if (seen.has(node.node_id) || nodes.length >= limit)
            return;
    ...
    for (const [index, event] of events.entries()) {
        ...
        if (event.hook_event === "PreToolUse" && metadata.tool_use_id) {
            ...
            if (preTool)
                addEdge(fallbackEdge(eventNodeId(preTool.event_id), eventNode, "tool_result", event.session_id, index, event.timestamp));
        }
...
function fallbackEdge(fromNodeId, toNodeId, kind, sessionId, position, createdAt) {
    return {
...
export function getStoredSessionGraph(db, rawLogPath, sessionId, limit) {
    if (!db)
        return buildFallbackGraph(readRawEvents(rawLogPath).filter((event) => event.session_id === sessionId), limit);
...
export function getLatestCheckpoint(db, sessionId) {
    if (!db)
        return undefined;
...
function buildCheckpointMetadata(db, sessionId, eventCount) {
    if (!db)
...
export function derivedGraphNodeCounts(db) {
    return countMap(db, `
...
export function derivedGraphEdgeCounts(db) {
    return countMap(db, `
    WITH ordered_events AS (
