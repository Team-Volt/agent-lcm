...
import { isCodexLcmToolEvent, isSummaryHook } from "./storage-sessions.js";
import { SUMMARY_ALGORITHM_VERSION, SUMMARY_NODE_CHUNK_SIZE, SUMMARY_NODE_FANOUT, SUMMARY_NODE_MAX_DEPTH, SUMMARY_NODE_SOURCE_EVENT_LIMIT, SUMMARY_NODE_VERSION, buildCondensedSummaryNode, buildLeafSummaryNode, buildSessionMemorySummary, eventSignalText, isSummarySourceEvent, matchesQueryText, queryTermHitCount, rankSummaryNodesForContext, summaryNodeSearchText, summarySearchText, takeHeadTail, toFtsQueries, } from "./summary.js";
...
export function getSessionMemorySummary(db, rawLogPath, sessionId) {
    if (!db) {
...
export function getSummaryNodesForSession(db, sessionId, limit = 200) {
    if (!db)
        return [];
    return db.prepare(`
...
export function getTopSummaryNodesForSession(db, sessionId, limit = 3) {
    if (!db)
        return [];
    return db.prepare(`
    SELECT node_id, session_id, depth, summary_text, token_count, source_token_count, source_type,
...
export function getSummaryNodesForGraph(db, sessionId, limit = 50) {
    const cappedLimit = clampSummaryLimit(limit, 50, 500);
    const nodes = getSummaryNodesForSession(db, sessionId, 2_000);
    const byId = new Map(nodes.map((node) => [node.node_id, node]));
    const selected = new Map();
    const addWithLineage = (node) => {
        if (selected.has(node.node_id) || selected.size >= cappedLimit)
            return;
    ...
    const roots = [...nodes].sort((a, b) => b.depth - a.depth ||
    ...
    return [...selected.values()].sort((a, b) => a.depth - b.depth ||
        a.earliest_at.localeCompare(b.earliest_at) ||
        a.node_id.localeCompare(b.node_id));
}
export function searchSummaryNodes(db, args) {
...
export function getSummaryNode(db, nodeId) {
    if (!db)
        return undefined;
    const row = db.prepare(`
    SELECT node_id, session_id, depth, summary_text, token_count, source_token_count, source_type,
...
export function getSourceSummaryNodes(db, node, limit = 4) {
    if (node.source_type !== "nodes")
        return [];
...
export function getSummaryNodeSourceEvents(db, node, query = "", limit = SUMMARY_NODE_SOURCE_EVENT_LIMIT) {
    if (!db)
        return [];
  ...
  `).all(...selectedIds);
    return rows
        ...
        .sort((a, b) => queryTermHitCount(eventSignalText(b), query) - queryTermHitCount(eventSignalText(a), query) ||
        a.timestamp.localeCompare(b.timestamp) ||
        a.event_id.localeCompare(b.event_id))
        .slice(0, clampSummaryLimit(limit, SUMMARY_NODE_SOURCE_EVENT_LIMIT, 20));
}
export function getSessionSummarySourceEvents(db, summary, query, limit) {
    if (!db || summary.source_event_ids.length === 0)
        return [];
...
export function getSummaryBackfillSessionIds(db) {
    if (!db)
        return [];
    return db.prepare(`
    SELECT s.session_id
...
export function shouldRebuildSessionMemorySummary(db, event) {
    if (!db || !isSummarySourceEvent(event))
        return false;
...
export function rebuildSessionMemorySummary(db, sessionId) {
    if (!db)
        return;
  ...
  `).run(summary.session_id, SUMMARY_ALGORITHM_VERSION, summary.updated_at, summary.cwd, summary.repo_root ?? null, summary.git_branch ?? null, summary.title, summary.overview, JSON.stringify(summary.topics), JSON.stringify(summary.key_prompts), JSON.stringify(summary.outcomes), JSON.stringify(summary.tools), JSON.stringify(summary.source_event_ids), summaryText);
    db.prepare(`
...
function rebuildSummaryNodes(db, sessionId) {
  ...
  `).all(sessionId);
    ...
    for (const node of nodes) {
        ...
        if (existingVersion === SUMMARY_NODE_VERSION)
            continue;
...
function insertSummaryNode(db, node) {
    db.prepare(`
    ...
    ON CONFLICT(node_id) DO UPDATE SET
      ...
      git_branch = excluded.git_branch,
      topics_json = excluded.topics_json
  `).run(node.node_id, node.session_id, SUMMARY_NODE_VERSION, node.depth, node.summary_text, node.token_count, node.source_token_count, node.source_type, JSON.stringify(node.source_ids), JSON.stringify(node.source_event_ids), node.earliest_at, node.latest_at, node.created_at, node.cwd, node.repo_root ?? null, node.git_branch ?? null, JSON.stringify(node.topics));
    db.prepare(`
    INSERT INTO summary_node_fts (node_id, session_id, cwd, repo_root, depth, content)
    VALUES (?1, ?2, ?3, ?4, ?5, ?6)
  `).run(node.node_id, node.session_id, node.cwd, node.repo_root ?? "", String(node.depth), summaryNodeSearchText(node));
}
function getAllSummarySourceEventsForSession(db, sessionId) {
    return db.prepare(`
    SELECT ${STORED_EVENT_JSON_SQL} AS raw_json FROM events
    WHERE session_id = ?1
...
function getSummaryEventsForSession(db, sessionId) {
...
function uniqueSummaryEvents(events) {
    const seen = new Set();
...
function clampSummaryLimit(limit, fallback, max = 200) {
    return Math.min(Math.max(Number(limit ?? fallback), 1), max);
}
function chunkSummaryValues(values, size) {
    const chunks = [];
