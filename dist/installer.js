...
import { codexHome, pluginRoot } from "./config.js";
export function readStatus(options = {}) {
    ...
    const marketplaceConfigured = configText !== undefined && /\[marketplaces\.agent-lcm\]/u.test(configText);
...
function pluginSectionEnabled(configText) {
    const lines = configText.split(/\r?\n/u);
    const start = lines.findIndex((line) => line.trim() === '[plugins."agent-lcm@agent-lcm"]');
    if (start < 0)
        return false;
    for (const line of lines.slice(start + 1)) {
        ...
        if (trimmed.startsWith("["))
            break;
        if (/^enabled\s*=\s*false(?:\s*#.*)?$/u.test(trimmed))
            return false;
    }
    return true;
}
