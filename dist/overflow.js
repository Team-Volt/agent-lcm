...
export function overflowReferenceFromEvent(event) {
...
function readVerifiedOverflowBuffer(args) {
    ...
    try {
        ...
        if (sha256(buffer) !== args.reference.sha256)
            throw new Error("Overflow payload integrity check failed.");
        return buffer;
    }
    finally {
        fs.closeSync(file);
...
function previousUtf8Boundary(buffer, offset) {
    let index = offset;
    while (index > 0 && index < buffer.length && (buffer[index] & 0xc0) === 0x80)
        index -= 1;
    return index;
}
