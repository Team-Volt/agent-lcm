...
function finishHookUpdate(action, harness, nativeStatus, target, update) {
    try {
        ...
        if (error instanceof SetupConfigurationChangedError) {
            ...
            throw new Error(`${result}, but Agent LCM detected a concurrent change to ${target} and did not overwrite it. `
                + `Repair it if needed, then rerun agent-lcm ${action} ${harness}.`, { cause: error });
        }
