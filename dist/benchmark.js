...
const RETRIEVAL_BENCHMARK_CWD = "/tmp/agent-lcm-retrieval-quality";
...
const RETRIEVAL_PARAPHRASE_FIXTURES = [
...
const RETRIEVAL_CORPUS = [
    ...
    ...RETRIEVAL_PARAPHRASE_FIXTURES.map(([sessionId, prompt]) => [sessionId, prompt]),
];
...
export function runLongContextBenchmark(options = {}) {
    ...
    const home = options.home ?? fs.mkdtempSync(path.join(os.tmpdir(), "agent-lcm-benchmark-"));
...
export function runRetrievalQualityBenchmark(options = {}) {
    const home = options.home ?? fs.mkdtempSync(path.join(os.tmpdir(), "agent-lcm-retrieval-quality-"));
    const cleanup = options.home === undefined;
    const startedAt = performance.now();
    const storage = createStorage({ home });
    try {
...
function retrievalMetrics(cases) {
