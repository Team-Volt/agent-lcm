...
export function rollbackPreservingError(db, original) {
    try {
        db?.exec("ROLLBACK");
        ...
        if (rollbackError instanceof Error)
            return { kind: "error", original, rollbackError };
...
export function previewCleanupReport(indexPath, inspection) {
    return {
        applied: false, raw_log_preserved: true, index_path: indexPath,
...
export function optimizeIndex(db) {
...
export function appliedCleanupReport(db, indexPath, inspection) {
    return {
...
export function writableIndexHealth(db, config, indexError, graphNodeCounts, graphEdgeCounts) {
    return {
...
export function rawHealth(config, indexError) {
...
export function knownEventIds(db, rawLogPath, eventIds) {
...
export function indexedEventsById(db) {
...
function parsedIndexedRawLogState(db) {
    const state = indexedRawLogState(db);
    if (!state)
        return undefined;
    try {
        const parsed = JSON.parse(state);
        if (typeof parsed !== "object" || parsed === null || typeof Reflect.get(parsed, "size") !== "number")
            return undefined;
...
export function segmentsNeedRawJsonClearing(db, segmentIds) {
...
function indexFileRefsForEvent(db, event) {
    ...
    `).run(ref.file_ref_id, ref.session_id, ref.observed_event_id, ref.timestamp, ref.path, ref.mime_type, ref.byte_count, ref.sha256, ref.exploration_summary, JSON.stringify(ref.metadata));
    }
}
function backfillExistingEventMetadata(db) {
  ...
  `);
    ...
    try {
        for (const row of rows) {
            ...
            const metadata = extractEventMetadata(event);
            update.run(metadata.turn_id ?? null, metadata.tool_use_id ?? null, eventAgentId(event) ?? null, overflowReferenceFromEvent(event)?.sha256 ?? null, event.event_id);
        }
        db.prepare(`
...
export function backfillLocatorMetadata(db) {
...
function backfillExistingSessionMetadata(db) {
...
export function backfillDelegationParents(db) {
    if (!db)
        return undefined;
  ...
  `).all();
    ...
    try {
        for (const row of rows) {
            ...
            if (parentId && parentId !== event.session_id)
                update.run(event.session_id, parentId);
        }
        db.prepare(`
      INSERT INTO index_metadata (key, value) VALUES (?1, '1')
      ON CONFLICT(key) DO UPDATE SET value = excluded.value
...
export function backfillFileRefs(db) {
    if (!db)
        return undefined;
  ...
  `).all();
    ...
    try {
        for (const row of rows)
            indexFileRefsForEvent(db, decodePersistedEvent(String(recordValue(row).raw_json)));
        db.prepare(`
      INSERT INTO index_metadata (key, value) VALUES (?1, '1')
      ON CONFLICT(key) DO UPDATE SET value = excluded.value
...
export function backfillSessionMemorySummaries(db) {
    if (!db)
        return undefined;
    ...
    try {
        for (const sessionId of sessionIds)
            rebuildSessionMemorySummary(db, sessionId);
        db.exec("COMMIT");
        return undefined;
    }
    catch (error) {
...
function outdatedSummarySessionIds(db) {
    return db.prepare(`
