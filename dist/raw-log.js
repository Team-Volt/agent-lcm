...
import { readManifest, segmentStoreState, segmentTimestampBounds, writeManifestAtomic } from "./raw-segments.js";
...
export class RawLogLockTimeoutError extends Error {
    ...
    constructor(lockPath) {
        super(`agent-lcm: raw log lock timeout: ${lockPath}`);
        this.name = "RawLogLockTimeoutError";
        this.lockPath = lockPath;
    }
}
export function withRawLogLock(rawLogPath, callback) {
...
function waitForRawLogLock(deadline, lockPath) {
...
export function appendRawEvents(rawLogPath, events) {
    if (events.length === 0)
        return;
    fs.mkdirSync(path.dirname(rawLogPath), { recursive: true, mode: 0o700 });
...
export function appendSegmentedEvents(config, events, options = {}) {
    if (events.length === 0)
        return [];
...
export function* readAllRawEvents(config) {
    for (const located of readAllLocatedRawEvents(config))
        yield located.event;
}
export function* readAllLocatedRawEvents(config) {
    const manifest = readManifest(config.manifestPath);
    if (manifest.migration?.complete === false) {
        for (const event of readRawFileEvents(relativeStorePath(config, manifest.migration.legacy_path)))
            yield { event };
...
export function* readActiveRawEvents(config) {
    yield* readRawFileLocatedEvents(config.rawLogPath, activeSegmentId(config));
}
export function readAllRawLog(config) {
    const events = [];
...
export function readLocatedEvent(config, location) {
    return createLocatedEventReader(config)(location);
}
export function createLocatedEventReader(config) {
    let cachedSegmentId;
    let cachedContent;
    return (location) => {
        ...
        if (!targetPath)
            throw new Error(`Unknown raw segment: ${location.segmentId}`);
        ...
        if (record?.compressed) {
            ...
            if (!content)
                throw new Error(`Compressed raw segment could not be read: ${record.id}`);
            ...
            if (!event)
                throw new Error("Raw event location does not contain a persisted event.");
            return event;
        }
...
function restoreRawLog(rawLogPath, existed, previousSize) {
    if (existed) {
...
function rawLogNeedsSeparator(rawLogPath) {
    if (!fs.existsSync(rawLogPath))
        return false;
...
function fsyncPath(targetPath, writable = false) {
...
export function readRawLog(rawLogPath) {
    if (!fs.existsSync(rawLogPath))
...
export function readRawEvents(rawLogPath) {
...
function rotateActiveRawLog(config) {
    ...
    if (fs.existsSync(destinationPath))
        throw new Error(`Raw segment already exists: ${id}`);
...
function activeSegmentId(config) {
    return nextSegmentId(readManifest(config.manifestPath).segments);
}
function segmentPath(config, record) {
...
function relativeStorePath(config, relativePath) {
...
function summarizeRawFile(rawLogPath) {
...
function hashRawFile(rawLogPath) {
    const descriptor = fs.openSync(rawLogPath, "r");
...
function* readRawFileEvents(rawLogPath) {
    if (!fs.existsSync(rawLogPath))
        return;
    for (const line of readRawFileLines(rawLogPath)) {
        if (line.trim().length === 0)
...
function* readRawFileLocatedEvents(rawLogPath, segmentId) {
    if (!fs.existsSync(rawLogPath))
        return;
...
function* readRawBufferLocatedEvents(content, segmentId) {
    let lineStart = 0;
    for (let index = 0; index <= content.length; index += 1) {
        ...
        if (line.trim().length > 0) {
            const event = parsePersistedEvent(line);
            if (event)
                yield { event, location: { segmentId, offset: lineStart, length } };
        }
        lineStart = index + 1;
    }
}
function* readRawFileLines(rawLogPath) {
    for (const record of readRawFileLineRecords(rawLogPath))
        yield record.line;
}
function* readRawFileLineRecords(rawLogPath) {
    const descriptor = fs.openSync(rawLogPath, "r");
    const buffer = Buffer.allocUnsafe(RAW_READ_BUFFER_BYTES);
