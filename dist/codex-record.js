...
import { contentSourceText, contentText, isCrossShapeMessageDuplicate, isSyntheticContextText, } from "./codex-message.js";
import { normalizeHookEvent } from "./events.js";
export function codexRecordToEvent(record, file, state) {
    ...
    if (type === "event_msg") {
        ...
        if (!role || text.length === 0 || isSyntheticContextText(text))
            return undefined;
        if (isCrossShapeMessageDuplicate(state, "event_msg", role, sourceText))
            return undefined;
        return normalizeImportEvent({
            ...
            sessionId: state.sessionId || sessionIdFromFile(file),
            cwd: state.cwd || "",
            payload: basePayload(file, type, timestamp, state, role === "user" ? { prompt: text } : { last_assistant_message: text }),
            state,
        });
    }
    ...
    if (itemType === "message") {
        ...
        if (text.length === 0 || isSyntheticContextText(text))
            return undefined;
        if (role === "user") {
            if (isCrossShapeMessageDuplicate(state, "response_item", "user", sourceText))
                return undefined;
            return normalizeImportEvent({
        ...
        if (role === "assistant") {
            if (isCrossShapeMessageDuplicate(state, "response_item", "assistant", sourceText))
                return undefined;
            return normalizeImportEvent({
                hookEvent: "Stop",
                timestamp,
...
function basePayload(file, type, timestamp, state, fields) {
    return {
        ...fields,
        imported_from: file,
        codex_record_type: type,
...
export function rolloutSessionIdFromFile(file) {
