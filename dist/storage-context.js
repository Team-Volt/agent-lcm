...
import { overflowReferenceFromEvent, readOverflowContent } from "./overflow.js";
import { readRawEvents } from "./raw-log.js";
import { recordValue, rowToFileReference } from "./storage-rows.js";
import { STORED_EVENT_JSON_SQL } from "./stored-event.js";
import { clampLimit, positiveInteger, searchStoredSessions, } from "./storage-search.js";
import { getCurrentStoredSession, getStoredSessionSummary, extractEventMetadata, } from "./storage-sessions.js";
import { getSessionMemorySummary, getSessionSummarySourceEvents, getSourceSummaryNodes, getSummaryNode, getSummaryNodesForSession, getSummaryNodeSourceEvents, getTopSummaryNodesForSession, searchSummaryNodes, } from "./storage-summaries.js";
import { harnessSet, matchesHarness, } from "./storage-types.js";
import { HISTORICAL_SOURCE_TEXT_NOTICE, SUMMARY_NODE_SOURCE_EVENT_LIMIT, estimateTokenCount, eventSignalText, matchesQueryText, queryTermHitCount, quoteHistoricalText, rankSummaryNodesForContext, summaryNodeExpansionToMarkdown, summaryNodeSearchText, summaryNodeTitle, summaryNodeToMarkdown, summarySearchText, } from "./summary.js";
...
function contextPlanState(estimatedRecentTokens, autoCompactTokenLimit, modelContextWindow) {
    if (estimatedRecentTokens <= 0)
        return "empty";
    if (estimatedRecentTokens >= modelContextWindow)
        return "over_context";
    if (estimatedRecentTokens >= autoCompactTokenLimit)
        return "over_limit";
    if (estimatedRecentTokens >= Math.floor(autoCompactTokenLimit * 0.8))
        return "near_limit";
    return "under_limit";
}
function contextPlanRecommendation(state, summaryNodeCount) {
    if (state === "empty")
        return "No matching session found.";
    if (state === "under_limit")
        return "No context packing needed yet.";
    if (summaryNodeCount === 0)
        return "Context pressure is high, but no summary nodes are available yet.";
    if (state === "near_limit")
        return "Near the soft context limit; use lcm_pack_context for broad recall before continuing.";
    if (state === "over_context")
        return "Estimated recent context is past the model window; use lcm_pack_context or lcm_expand_query for focused recovery.";
    return "Past the soft context limit; use lcm_pack_context or lcm_expand_query before relying on raw recent context.";
}
function rankQueryExpansionNodes(nodes, query, overview) {
    const ranked = rankSummaryNodesForContext(nodes, query);
    if (!overview)
        return ranked;
    return ranked.sort((left, right) => right.depth - left.depth ||
...
function focusedExcerpt(value, query, maxChars) {
    if (maxChars <= 0)
        return "";
...
export function parseTimestamp(value, name) {
    if (!value)
        return undefined;
...
export function getContextPlan(db, rawLogPath, args = {}) {
    const modelContextWindow = positiveInteger(args.modelContextWindow, DEFAULT_MODEL_CONTEXT_WINDOW);
    const autoCompactTokenLimit = Math.min(positiveInteger(args.autoCompactTokenLimit, DEFAULT_AUTO_COMPACT_TOKEN_LIMIT), modelContextWindow);
...
export function getFileRefsForSession(db, sessionId, limit = 50) {
    if (!db)
        return [];
    return db.prepare(`
...
export function getFileRef(db, fileRefId) {
    if (!db)
        return undefined;
...
export function getOverflowRef(db, rawLogPath, fileRefId) {
    if (!fileRefId.startsWith("overflow:"))
        return undefined;
...
export function describeMemory(db, rawLogPath, overflowDir, args) {
    if (args.fileId) {
        if (args.fileId.startsWith("overflow:")) {
            const reference = getOverflowRef(db, rawLogPath, args.fileId);
            if (!reference)
                throw new Error(`Overflow reference not found: ${args.fileId}`);
            return {
                target: "overflow_ref",
        ...
        if (!fileRef)
            throw new Error(`File reference not found: ${args.fileId}`);
        return {
            target: "file_ref",
            file_ref: fileRef,
        };
    ...
    if (args.nodeId) {
        ...
        if (!node)
            throw new Error(`Summary node not found: ${args.nodeId}`);
        return {
            target: "summary_node",
            node,
...
export function expandMemory(db, args) {
    const node = findSummaryNode(db, args.nodeId);
    if (!node)
        throw new Error(`Summary node not found: ${args.nodeId}`);
...
export function expandQuery(db, rawLogPath, args) {
    ...
    const sessionHarness = (sessionId) => getStoredSessionSummary(db, rawLogPath, sessionId)?.harness ?? "codex";
    const hasHarness = (sessionId) => !selectedHarnesses || selectedHarnesses.has(sessionHarness(sessionId));
    ...
    if (candidates.length === 0 && args.sessionIds?.length) {
        for (const sessionId of args.sessionIds) {
            ...
            if (!summary || (args.cwd && summary.cwd !== args.cwd) || (args.repoRoot && summary.repo_root !== args.repoRoot))
                continue;
            if (!matchesQueryText(summarySearchText(summary), query))
                continue;
            if (!hasHarness(sessionId))
                continue;
    ...
    const visit = (node) => {
        if (nodesById.has(node.node_id) || nodesById.size >= maxNodes)
            return;
    ...
    for (const candidate of rankQueryExpansionNodes(candidates, query, args.overview === true).slice(0, candidateLimit))
        visit(candidate);
    const nodes = rankQueryExpansionNodes([...nodesById.values()], query, args.overview === true);
    const events = [...eventsById.values()].sort((a, b) => queryTermHitCount(eventSignalText(b), query) - queryTermHitCount(eventSignalText(a), query) ||
    ...
    const addFocusedEventFallback = (event) => {
        let prefix = [
...
function topSummaryNodesForSession(db, sessionId, limit = 3) {
    return getTopSummaryNodesForSession(db, sessionId, limit);
}
function findSummaryNodes(db, args) {
    return searchSummaryNodes(db, args);
}
function findSummaryNode(db, nodeId) {
    return getSummaryNode(db, nodeId);
}
function sourceSummaryNodes(db, node, limit = 4) {
    return getSourceSummaryNodes(db, node, limit);
}
function summaryNodeSourceEvents(db, node, query = "", limit = SUMMARY_NODE_SOURCE_EVENT_LIMIT) {
    return getSummaryNodeSourceEvents(db, node, query, limit);
}
function sessionSummarySourceEvents(db, summary, query, limit) {
    return getSessionSummarySourceEvents(db, summary, query, limit);
}
function getContextPlanEvents(db, rawLogPath, sessionId, limit) {
    if (!db) {
        return readRawEvents(rawLogPath)
...
function getContextPlanSummaryStats(db, sessionId) {
    if (!db)
        return { summaryNodeCount: 0, estimatedSummaryTokens: 0 };
