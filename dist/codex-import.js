...
export async function readCodexSessions(source, onRecord) {
    const report = { files_scanned: 0, records_read: 0, records_skipped: 0, errors: [] };
    const files = listJsonlFiles(path.resolve(source));
    if (files.length === 0)
        report.errors.push({ file: path.resolve(source), message: `No JSONL session files found at ${source}` });
...
async function readCodexSessionFile(file, report, onRecord) {
    const state = {};
...
export function listJsonlFiles(source) {
    if (!fs.existsSync(source))
        return [];
    const stat = fs.statSync(source);
    if (stat.isFile())
        return source.endsWith(".jsonl") ? [source] : [];
    const result = [];
    const stack = [source];
