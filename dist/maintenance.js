...
import { parsePersistedEvent } from "./event-codec.js";
import { segmentedRawLogState, withRawLogLock } from "./raw-log.js";
import { emptySegmentManifest, readManifest, segmentTimestampBounds, writeManifestAtomic } from "./raw-segments.js";
import { backfillLocatorMetadata, clearVerifiedRawJson, indexEventInTransaction, invalidateRawLogState, recordRawLogState, segmentsNeedRawJsonClearing, } from "./storage-persistence.js";
...
export function cutOverLegacyLog(config) {
    return withRawLogLock(config.rawLogPath, () => {
        ...
        try {
            ...
            if (fs.existsSync(config.rawLogPath) && fs.statSync(config.rawLogPath).size === 0)
                fs.unlinkSync(config.rawLogPath);
            fs.renameSync(legacyPath, config.rawLogPath);
            throw error;
        }
    });
}
export function migrationInProgress(config) {
    if (!fs.existsSync(config.manifestPath))
        return false;
    return readManifest(config.manifestPath).migration?.complete === false;
}
export function runMaintenanceOnce(config, options = {}) {
    const empty = { migrated: 0, compressed: 0, expired: 0, cleared: 0, quarantined: 0, errors: [] };
    if (!fs.existsSync(config.manifestPath))
        return empty;
    ...
    try {
        ...
        try {
            ...
            if (error instanceof Error && Reflect.get(error, "errcode") === 5)
                return empty;
            throw error;
        }
        const migration = migrateLegacy(config, options);
        if (migrationInProgress(config) || migration.errors.length > 0)
            return migration;
...
export function maintenanceNeeded(config) {
    if (!fs.existsSync(config.manifestPath))
        return false;
...
function migrateLegacy(config, options) {
    ...
    if (!Number.isSafeInteger(segmentCapBytes) || segmentCapBytes <= 0)
        throw new TypeError("Segment cap must be a positive integer.");
    ...
    try {
        for (;;) {
            ...
            if (!state || state.complete || segmentsWritten >= maxSegments)
                break;
            const batch = readMigrationBatch(legacyPath, state.offset, segmentCapBytes, db);
            for (const record of batch.quarantines)
                writeQuarantine(config, record);
...
function maintainSegments(config, now) {
    let cleared = 0;
    let compressed = 0;
    const errors = [];
    const db = fs.existsSync(config.indexPath) ? new DatabaseSync(config.indexPath, { timeout: 5_000 }) : undefined;
        ...
        for (const record of readManifest(config.manifestPath).segments) {
            try {
                if (record.compressed)
                    verifyArchivedSegment(config, record);
                else {
            ...
            if (db)
                cleared += clearVerifiedRawJson(db, record.id);
        }
...
function expireSegments(config, db, now) {
    if (config.configError)
        return { expired: 0, errors: [config.configError] };
    ...
    if (db)
        removeOrphanOverflow(config, db);
    return { expired: expired.length, errors: [] };
}
function removeOrphanOverflow(config, db) {
    if (!fs.existsSync(config.overflowDir))
        return;
...
function compressSegment(config, record) {
...
function verifyArchivedSegment(config, record) {
    verifySegmentContent(gunzipSync(fs.readFileSync(path.join(config.home, record.path))), record);
}
function verifySegmentContent(content, record) {
    if (hashBuffer(content) !== record.sha256)
...
function readMigrationBatch(legacyPath, startOffset, segmentCapBytes, db) {
        ...
        for (;;) {
            ...
            if (bytesRead === 0) {
                if (pending.length > 0) {
                    ...
                    if (!result.added)
                        return { events, quarantines, nextOffset: pendingOffset, eof: false };
            ...
            for (let index = 0; index < input.length; index += 1) {
                ...
                if (!result.added)
                    return { events, quarantines, nextOffset: lineOffset, eof: false };
                canonicalBytes = result.canonicalBytes;
                lineStart = index + 1;
...
function addLegacyLine(line, offset, events, quarantines, canonicalBytes, segmentCapBytes, db, newlineBytes = 0) {
    if (line.length === 0)
...
function repairMalformedEvent(line, db) {
    if (!db)
        return undefined;
...
function publishLegacySegment(config, existingSegments, events, nextOffset, db) {
    ...
    if (db) {
      ...
      `);
            for (const entry of events) {
                ...
                if (!indexed.inserted)
                    update.run(id, entry.offset, entry.serialized.length, entry.event.event_id);
            }
            db.exec("COMMIT");
        }
        catch (error) {
...
function writeSegmentFile(segmentPath, content, expectedHash) {
    if (fs.existsSync(segmentPath)) {
        if (hashFile(segmentPath) !== expectedHash)
            throw new Error(`Existing migration segment failed verification: ${segmentPath}`);
        return;
    }
...
function updateMigrationOffset(config, offset) {
    withRawLogLock(config.rawLogPath, () => {
        const manifest = readManifest(config.manifestPath);
        writeManifestAtomic(config.manifestPath, {
            ...manifest,
...
function finishMigration(config, offset, error) {
    withRawLogLock(config.rawLogPath, () => {
        const manifest = readManifest(config.manifestPath);
        writeManifestAtomic(config.manifestPath, {
            ...manifest,
...
function writeQuarantine(config, record) {
    ...
    if (!fs.existsSync(targetPath))
        fs.writeFileSync(targetPath, JSON.stringify(record), { mode: 0o600, flag: "wx" });
}
function quarantineCount(config) {
    const quarantineDir = path.join(config.segmentsDir, "quarantine");
