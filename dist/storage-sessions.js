...
import { HARNESS_NAMES, harnessSessionId } from "./events.js";
import { readRawEvents } from "./raw-log.js";
import { isRecord, recordValue, rowToSessionSummary } from "./storage-rows.js";
import { STORED_EVENT_JSON_SQL } from "./stored-event.js";
import { harnessSqlFragment, harnessSet } from "./storage-types.js";
import { buildSessionMemorySummary, eventSignalText, isGeneratedSuggestionEvent, isSummarySourceEvent } from "./summary.js";
export function sortedSessionIds(sessionIds) {
    return Array.from(new Set(sessionIds)).sort((left, right) => left.localeCompare(right));
}
function sessionListSummary(summary) {
    return {
...
function sessionsWithDescendants(allSessions, roots) {
    ...
    while (queue.length > 0) {
        ...
        if (!session || selected.has(session.session_id))
            continue;
...
export function extractSessionMetadata(event) {
...
export function parentSessionId(metadata) {
...
function maxOptional(current, incoming) {
...
function usageReportFromRow(row) {
    return {
        totals: {
...
export function storageStats(db, rawLogPath, health, graphNodeCounts, graphEdgeCounts) {
    if (!db) {
        return {
            ...health,
...
export function listStoredSessions(db, rawLogPath, args, limit, offset, since, until) {
    if (!db) {
    ...
    LIMIT ?7 OFFSET ?8
  `).all(since ?? null, until ?? null, args.cwd ?? null, args.repoRoot ?? null, args.rootsOnly ? 1 : 0, args.parentSessionId ?? null, limit + 1, offset, ...harnessFilter.values);
...
export function storedUsage(db, rawLogPath, args, since, until) {
    if (!db) {
        ...
        let sessions = allSessions
            ...
            .filter((session) => !selectedHarnesses || selectedHarnesses.has(session.harness));
        if (args.rootsOnly) {
            sessions = sessionsWithDescendants(selectedHarnesses ? allSessions.filter((session) => selectedHarnesses.has(session.harness)) : allSessions, sessions.filter((session) => !session.parent_session_id));
        }
    ...
    `).get(since ?? null, until ?? null, args.cwd ?? null, args.repoRoot ?? null, args.parentSessionId ?? null, ...harnessFilter.values);
  ...
  `).get(since ?? null, until ?? null, args.cwd ?? null, args.repoRoot ?? null, args.parentSessionId ?? null, ...harnessSqlFragment("harness", args.harnesses, 6).values);
    return usageReportFromRow(recordValue(row));
}
export function getCurrentStoredSession(db, rawLogPath, args) {
    if (args.sessionId)
        return getStoredSessionSummary(db, rawLogPath, args.sessionId);
    if (!db) {
...
export function resolveStoredSessionIdentifier(db, rawLogPath, identifier) {
...
export function getStoredSession(db, rawLogPath, sessionId, limit, offset) {
...
export function getStoredSessionSummary(db, rawLogPath, sessionId) {
    if (!db) {
...
export function scalar(db, sql) {
    if (!db)
        return 0;
    return Number(recordValue(db.prepare(sql).get()).count);
}
function optionalNumberScalar(db, sql) {
    if (!db)
        return null;
    const value = recordValue(db.prepare(sql).get()).value;
    return typeof value === "number" ? value : null;
}
function optionalStringScalar(db, sql) {
    if (!db)
        return null;
    const value = recordValue(db.prepare(sql).get()).value;
    return typeof value === "string" && value.length > 0 ? value : null;
}
export function countMap(db, sql) {
    if (!db)
        return {};
...
export function isCodexLcmToolEvent(event) {
