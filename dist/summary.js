...
export function toFtsQueries(query) {
...
export function queryTermHitCount(text, query) {
    return queryTermHitCountFromTerms(text, queryTerms(query));
}
export function rankSummaryNodesForContext(nodes, query) {
...
export function summaryNodeSearchText(node) {
    return [
        node.summary_text,
        ...node.topics,
...
export function summaryNodeTitle(node) {
    const lines = node.summary_text.split(/\r?\n/u);
    const distinctiveSourceLine = lines.find((line) => line.startsWith("- ") && /[A-Z0-9][A-Z0-9_-]{3,}/u.test(line));
    const focusLine = lines.find((line) => line.startsWith("Focus:"));
    return titleFromText(distinctiveSourceLine?.replace(/^-\s*/u, "") ??
...
export function quoteHistoricalText(text, indent = "") {
    return text
        .split(/\r?\n/u)
        .map((line) => `${indent}> ${line}`)
        .join("\n");
}
export function summaryNodeToMarkdown(node) {
    return [
...
export function summaryNodeToCompactMarkdown(node, args) {
    const lines = [
    ...
    if (node.topics.length > 0)
        lines.push(`Topics: ${node.topics.slice(0, 8).join(", ")}`);
...
function prioritizeSummaryText(text, query) {
...
export function sessionSummaryToMarkdown(summary) {
    ...
    if (summary.topics.length > 0)
        lines.push(`Topics: ${summary.topics.slice(0, 16).join(", ")}`);
    if (displayedPrompts.length > 0)
        lines.push(`Key prompts: ${displayedPrompts.join(" | ")}`);
    if (summary.outcomes.length > 0)
        lines.push(`Outcomes: ${summary.outcomes.slice(0, 2).join(" | ")}`);
    if (summary.source_event_ids.length > 0)
        lines.push(`Sources: ${summary.source_event_ids.slice(0, 3).map(shortSourceId).join(", ")}`);
    lines.push("");
    return lines.join("\n");
}
export function summarySearchText(summary) {
    return [
        summary.title,
        summary.overview,
        ...summary.topics,
...
export function buildLeafSummaryNode(events) {
...
export function buildCondensedSummaryNode(nodes, depth) {
...
export function buildSessionMemorySummary(events) {
    ...
    const tools = uniqueStrings(sorted
        ...
        .filter((tool) => typeof tool === "string" && tool.length > 0 && !tool.startsWith("mcp__agent_lcm__")))
        .slice(0, 8);
...
export function eventSignalText(event) {
...
export function isSummarySourceEvent(event) {
    if (isGeneratedSuggestionEvent(event))
        return false;
    const signal = compactWhitespace(eventSignalText(event));
    if (event.hook_event === "UserPromptSubmit") {
        if (/^<heartbeat>/iu.test(signal))
            return false;
        if (/^<recommended_plugins>/iu.test(signal))
            return false;
    }
    if (event.hook_event === "Stop") {
        if (event.payload.stop_hook_active === true)
            return false;
        if (/^<heartbeat>/iu.test(signal) && /<decision>\s*DONT_NOTIFY\s*<\/decision>/iu.test(signal))
            return false;
    }
...
export function isGeneratedSuggestionEvent(event) {
...
function isSuggestionJson(text) {
...
export function takeHeadTail(values, limit, headCount) {
...
function toStrictFtsQuery(query) {
    return query
        .split(/\s+/u)
        .map(quoteFtsTerm)
        .join(" AND ");
}
function toRelaxedFtsQuery(query) {
    const terms = queryTerms(query);
    if (terms.length < 2)
        return undefined;
...
function isSignalTerm(term) {
    if (term.length < 2)
        return false;
    if (COMMON_FUNCTION_TERMS.has(term))
        return false;
    if (/^\d+$/u.test(term))
        return term.length >= 2;
    if (term.includes("-") || /\d/u.test(term))
        return true;
    return term.length >= 3;
}
function summaryNodeRank(node, terms) {
    return {
...
function queryTermHitCountFromTerms(text, terms) {
    const lowered = text.toLowerCase();
    return terms.filter((term) => textContainsTerm(lowered, term)).length;
}
function textContainsTerm(loweredText, term) {
    if (/^\d+$/u.test(term)) {
...
function summaryNodeFocusedSearchText(node) {
    return [
        summaryNodeTitle(node),
        ...node.topics,
    ].join("\n");
}
function summaryNodeDirectness(node, terms) {
    if (terms.length === 0)
        return 0;
...
function directnessScore(loweredText, terms, uniqueHitWeight) {
    let uniqueHits = 0;
    let totalHits = 0;
...
function termOccurrenceCount(loweredText, term) {
    if (/^\d+$/u.test(term))
        return numericTermOccurrenceCount(loweredText, term);
...
function numericTermOccurrenceCount(loweredText, term) {
...
function summaryNodeEvidenceSupport(node) {
...
function overviewFromSignals(prompts, outcomes) {
    const parts = [];
    if (prompts.at(-1))
        parts.push(`User focus: ${truncateText(compactWhitespace(prompts.at(-1) ?? ""), 220)}`);
    if (outcomes[0])
        parts.push(`Latest outcome: ${truncateText(compactWhitespace(outcomes.at(-1) ?? outcomes[0]), 220)}`);
...
function extractTopics(inputSignals) {
    ...
    return [...stats.entries()]
        .sort((a, b) => topicScore(b[0], b[1], latestSignalIndex) - topicScore(a[0], a[1], latestSignalIndex) ||
        b[1].lastSeen - a[1].lastSeen ||
        a[1].firstSeen - b[1].firstSeen)
        .map(([term]) => term)
        .slice(0, 16);
}
function topicScore(term, stat, latestSignalIndex) {
...
function shortSourceId(eventId) {
    return eventId.length > 12 ? eventId.slice(0, 12) : eventId;
}
function summaryNodeId(sessionId, depth, sourceType, sourceIds) {
