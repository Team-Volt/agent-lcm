...
function acquireLock(name) {
    ...
    if (status.isSymbolicLink())
        throw new Error(`Refusing setup lock symlink: ${path.resolve(lock)}`);
    if (!status.isDirectory())
        throw new Error(`Cannot use setup lock that is not a directory: ${path.resolve(lock)}`);
    return BUSY_EXIT;
}
function writeCurrentFile(name) {
...
function readAnchoredFile(name) {
    ...
    if (pathStatus.isSymbolicLink())
        throw new Error(`Refusing setup configuration symlink: ${path.resolve(name)}`);
    if (!pathStatus.isFile())
        throw new Error(`Cannot update setup configuration that is not a regular file: ${path.resolve(name)}`);
    let descriptor;
    try {
...
function writeAnchoredFile(name, bytes) {
...
function assertDirectoryIdentity(expectedDevice, expectedInode) {
...
function requiredArgument(index) {
    ...
    if (value === undefined)
        throw new Error("Missing setup file worker argument.");
    return value;
}
