...
export function extractFileReferences(event) {
...
function collectCandidates(value, location) {
    if (!isRecord(value)) {
...
function candidateFromRecord(record, location) {
...
function contentInfo(value, sourcePath) {
...
function candidateToRef(event, candidate) {
...
function summarizeContent(args) {
    ...
    if (!args.content)
        return `${args.mimeType} file at ${args.path}, ${size}.${truncation}`;
    if (args.mimeType === "application/json") {
        const json = safeJson(args.content);
        if (Array.isArray(json))
            return `JSON array file at ${args.path}, ${size}. Items: ${json.length}.${truncation}`;
        if (isRecord(json))
            return `JSON object file at ${args.path}, ${size}. Keys: ${Object.keys(json).slice(0, 8).join(", ") || "none"}.${truncation}`;
    }
...
function guessMimeType(filePath, content) {
    ...
    if ([".py", ".rb", ".go", ".rs", ".swift", ".java", ".kt", ".c", ".cc", ".cpp", ".h"].includes(ext))
        return "text/x-code";
    if ([".md", ".txt", ".log", ".yaml", ".yml", ".toml", ".sql", ".xml", ".html", ".css"].includes(ext))
        return "text/plain";
