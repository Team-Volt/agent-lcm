...
test("retention configuration reads valid .env values and rejects invalid values", () => {
  ...
  assert.equal(loadConfig({ home: validHome, env: { AGENT_LCM_RETENTION_DAYS: "30" } }).retentionDays, 30);

  for (const value of ["0", "-1", "2.5", "ninety"]) {
    const home = tempHome();
    fs.writeFileSync(path.join(home, ".env"), `AGENT_LCM_RETENTION_DAYS=${value}\n`);
    const invalid = loadConfig({ home, env: {} });
    assert.equal(invalid.retentionDays, undefined, value);
    assert.equal(invalid.configError, "AGENT_LCM_RETENTION_DAYS must be a positive integer.", value);
  }
  const unsafe = loadConfig({ home: tempHome(), env: { AGENT_LCM_RETENTION_DAYS: "9".repeat(400) } });
  assert.equal(unsafe.retentionDays, undefined);
  assert.equal(unsafe.configError, "AGENT_LCM_RETENTION_DAYS must be a positive safe integer.");

  const duplicateHome = tempHome();
  fs.writeFileSync(duplicateHome + "/.env", "AGENT_LCM_RETENTION_DAYS=90\nAGENT_LCM_RETENTION_DAYS=91\n");
...
test("invalid retention configuration blocks source deletion", () => {
  const home = tempHome();
  fs.writeFileSync(path.join(home, ".env"), "AGENT_LCM_RETENTION_DAYS=0\n", { mode: 0o600 });
  const config = loadConfig({ home });
  const event = normalizeHookEvent({
    hookEvent: "UserPromptSubmit",
  ...
  assert.deepEqual(report.errors, ["AGENT_LCM_RETENTION_DAYS must be a positive integer."]);
...
test("reopening indexed storage reconciles same-ID raw-log payload edits", () => {
  // Given
  const home = tempHome("agent-lcm-raw-prefix-edit-");
  const storage = createStorage({ home });
...
test("ingest restores a raw event removed while storage remains open", () => {
  // Given
  const home = tempHome("agent-lcm-live-raw-truncate-");
  const storage = createStorage({ home });
  const event = normalizeHookEvent({
    hookEvent: "UserPromptSubmit",
...
test("raw-log waiter times out without evicting an active owner after ten seconds", async () => {
  ...
  assert.match(waiter.stderr, /RawLogLockTimeoutError: agent-lcm: raw log lock timeout:/u);
...
test("overflow recovery refuses event-supplied paths outside managed storage", () => {
  ...
  const externalPath = path.join(tempHome("agent-lcm-external-overflow-"), "payload.json");
...
test("search sessions relaxes broad queries when strict FTS has no match", () => {
  ...
  const matches = storage.searchSessions({
    query: "how is agent-lcm working out retrieval quality plumbing intelligence layer",
    limit: 5,
  });
...
test("packed summaries expose latest matching prompts in long sessions", () => {
  ...
  for (let index = 0; index < 8; index += 1) {
    storage.ingest(normalizeHookEvent({
      ...
      rawInput: JSON.stringify({
        ...
        cwd,
        prompt: index === 7
          ? "LATEST-TAIL-PROMPT improve agent-lcm retrieval summaries"
          : `older prompt ${index}`,
      }),
      env: {},
      now: () => new Date(Date.UTC(2026, 5, 9, 12, 0, index)),
  ...
  assert.ok(summary);
  assert.deepEqual(summary.topics.slice(0, 2), ["latest-tail-prompt", "agent-lcm"]);
...
async function runConcurrentIngestWriters(
  ...
  const home = tempHome("agent-lcm-concurrent-ingest-");
  const rawLogPath = path.join(home, "events.jsonl");
