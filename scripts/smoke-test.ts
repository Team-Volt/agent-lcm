...
const home = fs.mkdtempSync(path.join(os.tmpdir(), "agent-lcm-smoke-"));
...
try {
  ...
  const responses = runMcp([
    ...
    },
    {
      ...
      params: {
        ...
        arguments: { sessionId: "codex:smoke-session", limit: 20 },
      },
    },
    {
      jsonrpc: "2.0",
  ...
  assert.doesNotMatch(responses[2].result.content[0].text, /mcp__agent_lcm__/u);
  ...
  assert.deepEqual(
    responses[5].result.structuredContent.matches.map((match: { harness: string }) => match.harness).sort(),
    ["codex", "cursor"],
  );
  ...
  const restarted = runMcp([
    { jsonrpc: "2.0", id: 1, method: "initialize", params: { protocolVersion: "2025-11-25" } },
    { jsonrpc: "2.0", id: 2, method: "tools/call", params: { name: "lcm_grep", arguments: { query: "smoke searchable context", limit: 5 } } },
  ]);
  assert.deepEqual(
...
function runHook(event: string, payload: unknown): void {
  const result = spawnSync(process.execPath, ["--no-warnings", "bin/agent-lcm", "hook", event], {
    cwd: root,
...
function runMcp(requests: unknown[]) {
  const result = spawnSync(process.execPath, ["--no-warnings", "bin/agent-lcm", "mcp"], {
    cwd: root,
